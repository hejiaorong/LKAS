...
set_param(cs,'ReadBeforeWriteMsg','EnableAllAsError');
set_param(cs,'WriteAfterReadMsg','EnableAllAsError');
set_param(cs,'WriteAfterWriteMsg','EnableAllAsError');
...
fprintf("2.Function Call SubSystem 创建完成\n");

%% 7.添加大模块Inport/Outport
for i=3:in_excel_num
...
for i=1:length(Inport)-1
    ...
    set_param(add_line(strcat(module_name,'/',module_name),h1.Outport(1),h2.Inport(1),'autorouting','on'),'name',input(i+2,2));
end
...
for i=1:length(Outport)
    ...
    set_param(add_line(strcat(module_name,'/',module_name),h.Outport(1),h1.Inport(1),'autorouting','on'),'name',output(i+2,2));
...
for i=3:in_excel_num
    ...
    mapSignal(slMap,h.Outport(1),'Auto');
end
for i=3:out_excel_num
